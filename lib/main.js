...
const ballOnField = () => {
  ...
  const drawScore = () => {
      ...
      ctx.fillStyle = "white";
      ctx.fillText(goalsPlayerOne, 5, 30);
      ctx.fillText(goalsPlayerTwo, w - 15, 30);
  };
  ...
  const score = () => {
    if (ballX > w - 20 - ballRadius && ballY + dy > 50 + ballRadius && ballY + dy < 90 + ballRadius) {
      goalsPlayerOne++;
    } else if (ballY + dy > 50 + ballRadius && ballY + dy < 90 + ballRadius && ballX + dx < 20 + ballRadius) {
      goalsPlayerTwo++;
    }
  };
